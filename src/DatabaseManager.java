...
public class DatabaseManager {
    ...
    private User loadUserInfoFromDataBase(int id) throws SQLException {
        //-----------------------------------------------user-MAIL is loaded--------------------------------------------------
    ...
    private ArrayList<MailBody> loadUserMailsFromDataBase(int id) throws SQLException {
        ...
        while (rs.next()) {
            ...
            mailBody.setSpam(rs.getBoolean(14));    //Spam value is checked
            ...
            mailBody.setTimeStamp(rs.getTimestamp(8));
    ...
    private ArrayList<MailBody> loadUserDraftMailsFromDataBase(int id) throws SQLException {
        ArrayList<MailBody> mails = new ArrayList<>();
        //--------------------------------------mails-MAIL is loaded---------------------------------------------------
    ...
    private ArrayList<MailBody> loadUserSentMailsFromDataBase(int id) throws SQLException {
        ArrayList<MailBody> mails = new ArrayList<>();
        //-----------------------------------------sent-MAIL is loaded------------------------------------
        ...
        while (rs.next()) {
            ...
            sentMail.setTimeStamp(rs.getTimestamp(8));
    ...
    public void updateUserDataBase(int id){
        try {
            ...
            updateUserMails();
            updateUserSentMails();
            updateUserDraftMails();
        }catch(SQLException e) {
            e.printStackTrace();
        }
    }

    private void updateUserDraftMails() throws SQLException {
        PreparedStatement ppStatement = connection.prepareStatement("UPDATE mail SET subject = ?,body = ?,multiple_recipients = ? WHERE id = ?");
        for (int i = 0; i < Driver.mail.getDraft().size(); i++) {
                ppStatement.setString(1,Driver.mail.getDraft().get(i).getSubject());
                ppStatement.setString(2,Driver.mail.getDraft().get(i).getText());
                ppStatement.setString(3,Driver.mail.getDraft().get(i).getMultiple_recipients());
                ppStatement.setInt(4,Driver.mail.getDraft().get(i).getId());
                ppStatement.executeUpdate();
        }
    }
    private void updateUserInfo(){
        try {
            PreparedStatement ppsStatement = connection.prepareStatement("UPDATE user SET password = ?,firstName=?,lastName=?,address=?,securityQuestion=?,securityQuestionAnswer=? WHERE id = ?");
            ppsStatement.setString(1, Driver.mail.getUser().getPassword());
            ppsStatement.setString(2, Driver.mail.getUser().getFirstName());
            ppsStatement.setString(3, Driver.mail.getUser().getLastName());
            ppsStatement.setString(4, Driver.mail.getUser().getAddress());
            ppsStatement.setString(5, Driver.mail.getUser().getSecurityQuestion());
            ppsStatement.setString(6, Driver.mail.getUser().getSecurityQuestionAnswer());
            ppsStatement.setInt(7, Driver.mail.getUser().getId());
            ppsStatement.executeUpdate();
        }catch(SQLException e)
        {
            e.printStackTrace();
    ...
    private void updateUserMails() throws SQLException {
        PreparedStatement ppsStatement = connection.prepareStatement("UPDATE mail SET status = ?,recipient_starred = ?,permaTrash = ?,spam=? WHERE id = ?");
        for (int i = 0; i < Driver.mail.getInbox().size(); i++) {
            int status;
            /*
            1-UnRead - Inbox
            2-Read - Inbox   //The mail cannot be read until it isn't in unread form first
            3-Draft  //Not included as these are inbox mails and they cannot be set Draft
            4-Trash
             */
            if (Driver.mail.getInbox().get(i).isUnread()) {
                status = 1;
            } else if (Driver.mail.getInbox().get(i).isTrash()) {
                status = 4;
            } else {
                status = 2;
            }

            ppsStatement.setInt(1, status);
            ppsStatement.setBoolean(2, Driver.mail.getInbox().get(i).isRecipient_starred());
            ppsStatement.setInt(3, Driver.mail.getInbox().get(i).getPermaTrash());
            ppsStatement.setBoolean(4,Driver.mail.getInbox().get(i).isSpam());
            ppsStatement.setInt(5,Driver.mail.getInbox().get(i).getId());
            ppsStatement.executeUpdate();
        }
    }
    private void updateUserSentMails() throws SQLException {
    ...
    public boolean checkSpam(String body) {
        for (String[] spamWord : spamWords) {
            if (body.toLowerCase().contains(spamWord[0].toLowerCase()) && body.contains(spamWord[1].toLowerCase()) && body.contains(spamWord[2].toLowerCase())) {
                return true;
            }
        }
        return false;
